...
namespace NetTransferService.Jobs
{
    ...
    public class PaymentJob : IJob
    {
        ...
        public PaymentJob(IServiceProvider serviceProvider, IConfiguration configuration)
        {
        ...
        public async Task Execute(IJobExecutionContext context)
        {
            ...
            {
                var logger = scope.ServiceProvider.GetRequiredService<ILogger<PaymentJob>>();

                logger.LogInformation($"Ödeme aktarım görevi başladı : {DateTime.Now}");
                ...
                await transfer.TahsilatTrasnfer();

                logger.LogInformation($"Ödeme aktarım görevi tamamlandı : {DateTime.Now}");
            }
        }
        public async Task<Transfer?> InitializeSettingAsync(ILogger<PaymentJob> logger)
        {
            try
            {
